...
class LeveragedETFProvider:
    ...
    def get_history(
        ...
        logger.info("")
        logger.info("RAW TBILL DATA FROM TREASURY PROVIDER")
        logger.info("-" * 60)

        logger.info(
            f"TBILL rows        : {len(treasury_rate)}"
        )

        logger.info(
            f"TBILL first date  : {treasury_rate.index.min()}"
        )

        logger.info(
            f"TBILL last date   : {treasury_rate.index.max()}"
        )

        logger.info(
            f"TBILL first value : {treasury_rate.iloc[0]:.12f}"
        )

        logger.info(
            f"TBILL last value  : {treasury_rate.iloc[-1]:.12f}"
        )

        logger.info(
            f"TBILL minimum     : {treasury_rate.min():.12f}"
        )

        logger.info(
            f"TBILL maximum     : {treasury_rate.max():.12f}"
        )

        logger.info("")
        logger.info("TBILL SAMPLE:")
        logger.info(treasury_rate.head(10).to_string())
        logger.info(treasury_rate.tail(10).to_string())
